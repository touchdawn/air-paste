    ...
    fn run_ui_and_paint(
        ...
        if is_visible && clear_before_update {
...

            {
                ...
                frame_timer.resume();
                if !context_ok {
                    // [PATCHED] Transient make-current failure: skip this frame and retry on
                    // a gentle cadence instead of panicking the main thread.
        ...
        if is_visible {
...

            {
                ...
                frame_timer.resume();
                if !context_ok {
                    // [PATCHED] Transient make-current failure: skip painting this frame and
                    // retry on a gentle cadence instead of panicking the main thread.
                    return Ok(EventResult::RepaintAt(
                        window.id(),
                        Instant::now() + Duration::from_millis(250),
                    ));
...

            {
                ...
                // [PATCHED] vs upstream, which propagated these errors with `?` — the event
                // loop treats that as fatal and exits the whole app. A transient swap failure
                // (lock screen, display sleep, GPU reset, RDP session switch) should instead
                // retry on a gentle cadence; a persistent one then degrades to a blank
                // window rather than a silently vanishing process.
                let Some(context) = current_gl_context.as_ref() else {
                    log::error!("failed to get current context to swap buffers");
                    return Ok(EventResult::RepaintAt(
                        window.id(),
                        Instant::now() + Duration::from_millis(250),
                    ));
                };
                if let Err(err) = gl_surface.swap_buffers(context) {
                    log::error!("swap_buffers failed (will retry): {err}");
                    frame_timer.resume();
                    return Ok(EventResult::RepaintAt(
                        window.id(),
                        Instant::now() + Duration::from_millis(250),
...
/// Make `gl_surface` current, returning `false` (instead of the upstream `.unwrap()` panic)
...
fn change_gl_context(
    ...
    if let Some(current) = current_gl_context.as_ref() {
        ...
        return match current.make_current(gl_surface) {
            Ok(()) => true,
            Err(err) => {
                log::error!("Failed to make GL context current (will retry next frame): {err}");
                false
            }
        };
    }

    if let Some(not_current) = not_current_gl_context.take() {
        profiling::scope!("make_current");
        return match not_current.make_current(gl_surface) {
            Ok(current) => {
    ...
    log::error!("No GL context available to make current");
    false
}
...
impl GlutinWindowContext {
    ...
    pub(crate) fn initialize_window(
        ...
        if viewport.gl_surface.is_none() {
            ...
            // [PATCHED] Make the new surface current via whichever context slot is populated.
            // The already-current branch uses the non-consuming
            // `PossiblyCurrentGlContext::make_current(&self, …)`, so a transient failure
            // (lock screen, display sleep, GPU reset, RDP session switch) propagates as an
            // error the callers already log — instead of the previous `.unwrap()` panic.
            if let Some(not_current_context) = self.not_current_gl_context.take() {
                self.current_gl_context = Some(not_current_context.make_current(&gl_surface)?);
            } else if let Some(current_gl_context) = self.current_gl_context.as_ref() {
                current_gl_context.make_current(&gl_surface)?;
            } else {
                log::error!("No GL context available while initializing a viewport window");
            }
    ...
    fn resize(&mut self, viewport_id: ViewportId, physical_size: winit::dpi::PhysicalSize<u32>) {
        ...
        {
            // [PATCHED] Skip the resize when the context cannot be made current; the next
            // successful frame resizes the surface to the window's then-current size anyway.
            if change_gl_context(
                &mut self.current_gl_context,
                &mut self.not_current_gl_context,
                gl_surface,
            ) && let Some(current_gl_context) = self.current_gl_context.as_ref()
            {
...
fn render_immediate_viewport(
    ...
    if !change_gl_context(current_gl_context, not_current_gl_context, gl_surface) {
        // [PATCHED] Transient make-current failure: drop this immediate-viewport frame
        // instead of panicking; the parent viewport repaints it on the next pass.
        return;
    }
