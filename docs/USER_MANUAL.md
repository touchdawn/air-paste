...
- **本机作为服务器**复选框:在本机直接跑一个控制平面服务器(默认 `0.0.0.0:8080`,数据库放
  `…/AirPaste/server.redb`),供其它设备连接,不用单独开 `airpaste-server`。勾选后状态栏显示运行中/失败;
  会持久化,下次启动自动起(且在 agent 连接前就绪)。本机 agent 默认就连 `127.0.0.1:8080`,所以勾上即用。
...
1. **A:** 打开 AirPaste 托盘 → 设置面板填服务器地址(本机作服务器就保持默认 `http://127.0.0.1:8080`)→
   勾 **本机作为服务器** → 点 **保存并连接**。A 是第一台,自动信任 → `● 已连接`。
   (macOS 记得在 系统设置 → 隐私与安全性 → 辅助功能 里授权启动托盘的程序,否则热键无效。)
2. **A:** 在设置面板点 **生成配对码**,记下那 6 位数字码。
3. **B:** 打开托盘 → 设置面板填服务器地址(A 的局域网 `http://A的IP:8080`)+ 把配对码填进 **配对码** →
