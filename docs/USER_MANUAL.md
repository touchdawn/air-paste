...
- **设置 / 连接** 面板:填**服务器地址 / 配对码 / 认证令牌**,点 **保存并连接**(配置持久化到
  ...
  面板里还有 **生成配对码** 按钮(已连接时可用):在**可信设备**上点一下出一个 8 位码(10 分钟有效),
  拿到新设备的「配对码」里填即可——**给新设备入网不用再碰命令行**。
- **收件箱**：隔离模式下收到的最近若干条文本，逐条可“复制”。隔离模式下文件也会进“待接收”而不直接覆盖系统剪贴板;窗口会显示“待接收文件”,`Ctrl+Shift+V` 会按**最近到达**优先(文本或文件)。下载多/大文件时窗口显示进度条。
...
2. **A:** 在设置面板点 **生成配对码**,记下那 8 位码。
3. **B:** 打开托盘 → 设置面板填服务器地址(A 的局域网 `http://A的IP:8080`)+ 把配对码填进 **配对码** →
   **保存并连接** → `● 已连接`。(macOS 同样授权辅助功能。)
4. **A 发:** 选中文字,按 `Ctrl+Shift+C`。**B 收:** 点进输入框,按 `Ctrl+Shift+V`。
   也可在 B 的「收件箱」点「复制」。文件/文件夹同理:在 A 复制,B 按 `Ctrl+Shift+V` 下载(带进度条)。
