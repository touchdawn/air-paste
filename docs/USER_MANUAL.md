...
- `airpaste-server`：控制平面服务器。负责设备注册、配对、可信设备列表、剪贴板元数据、WebSocket 通知、文本历史和中继会话元数据。
...
- 如果两台设备互相无法访问 peer 端口，当前版本的文件传输会失败，因为 relay 数据通道还没有实现。
