...
> 当前项目仍处于 MVP 开发阶段。本手册以“从源码编译并运行 CLI 程序”为准；暂时还没有图形界面、安装包、托盘程序、系统服务或开机自启动安装器。
...
- 没有 GUI、托盘图标、设置页、安装包或系统服务。
- 图片同步模型存在于协议里，但客户端当前主要实现文本和文件。
- 文件清单尚未端到端加密，服务器能看到文件名、大小、来源 peer URL 等元数据。
- 文件夹会出现在清单中，但当前只下载普通文件，不递归复制文件夹。
- Relay 中继数据通道已实现（端到端加密、服务器只转发不留存）：接收端加 `--prefer-relay` 时，文件会通过服务器中继传输，适合两台设备无法直连的情况。目前需要手动开启，尚未在直连失败时自动回退到中继。
...
- 如果两台设备互相无法访问 peer 端口，在接收端加 `--prefer-relay`，文件会改走服务器加密中继（双方都只向服务器发起出站连接，服务器只转发密文、不留存）。
