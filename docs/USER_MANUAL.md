...
Air Paste 是一个用 Rust 编写的跨设备剪贴板工具，支持 Windows、macOS，以及通过快捷指令接入的 iPhone（文本收发，无需装 App，见 `docs/IOS_SHORTCUTS.md`）。它的目标是让你的多台可信设备共享剪贴板：在设备 A 复制文本或文件，在设备 B 接收并粘贴。
...
- 隔离模式（托盘 GUI 默认）：`Alt+C` 把当前系统剪贴板发送到 AirPaste，`Alt+V` 从 AirPaste 收件箱粘贴；文本不与系统剪贴板自动同步，避免污染本机剪贴板。
- 远端文件粘贴热键：`Alt+V`（下载并粘贴待接收的远端文件）。
- macOS 和 Windows 客户端均支持文本、文件清单、文件下载和本地剪贴板写入。
- iPhone 通过快捷指令调用 `/v1/simple/*` 文本接口收发剪贴板（服务器收到上传后立即为各受信任设备端到端封装），配置方法见 `docs/IOS_SHORTCUTS.md`。
...
托盘 GUI 启动时默认进入**隔离模式**。此时文本**不会**随系统剪贴板自动同步，改由两个热键显式控制——既不污染本机剪贴板，也不会把每一次复制都外发：

- `Alt+C`（macOS 同一物理键即 `Option+C`）：把**当前系统剪贴板**里的文本端到端加密后发布到 AirPaste。用法是先正常 `Ctrl+C` / `Cmd+C` 复制，再按 `Alt+C` 发送。与上次发送内容相同时会自动跳过——连按两次只发一条，也不会误发剪贴板里的陈旧内容。
- `Alt+V`：从 AirPaste **收件箱**取最近一条文本粘贴到当前应用（粘贴时临时借用系统剪贴板，完成后还原）；若最近到达的是待接收文件，则改为下载并粘贴文件。

收到的远端文本只进 AirPaste 收件箱（可在窗口里查看和复制），不会自动写入系统剪贴板。

不想用热键时，托盘窗口里也能完成同样的事：输入文字点「发送」、把文件拖进窗口发送、在收件箱对文件条目点「下载」接收（见 14.2 节）。

> macOS 上 `Alt+V`（向其他应用模拟粘贴）需要在「系统设置 → 隐私与安全性 → 辅助功能」里授权；`Alt+C` 只是读取剪贴板，不需要该权限。

要恢复“复制即自动同步”的行为，可在窗口里取消「隔离模式」勾选，或用 `--clipboard-mode=system`（默认）启动 CLI agent。
...
5. 两台 agent 都保持运行。
6. 复制文本会自动同步。
7. 复制文件后，在接收端按 `Alt+V` 触发远端文件下载和粘贴准备。
...
- 接收远端文件清单，按 `Alt+V` 后下载文件并把文件 URL 写入 pasteboard。
...
```bash
target/debug/airpaste-agent \
  --server-url http://<server-host>:14444 \
  --device-name "MacBook" \
...
- `--server-url`：server 地址，默认 `http://127.0.0.1:14444`。
- `--auth-token`：server token。server 启用 token 时必填。
- `--device-name`：设备显示名(托盘里对应「设备名称」)。改名后下次启动会自动同步到 server,
  无需重新注册;留空用平台默认名(`Mac Agent` / `Windows Agent`)。
- `--hotkey-copy` / `--hotkey-paste`：自定义两个全局热键(托盘里对应「发送热键 / 粘贴热键」),
  默认 `alt+c` / `alt+v`。格式如 `ctrl+shift+f9`:至少一个修饰键(`alt`/`option`、`ctrl`、`shift`、
  `cmd`/`win`)加一个字母、数字或 F1-F12;两个热键不能相同。
...
- `--remote-paste-hotkey=false`：禁用 `Alt+V` 监听。
...
脚本会准备一个待处理文件 clip，然后等待你手动按 `Alt+V`。
...
- 发布本机复制的文本和文件清单。
- 接收远端文本并写入本机剪贴板。
- 接收远端文件清单，按 `Alt+V` 后下载文件、写入文件剪贴板，并模拟普通 `Ctrl+V`。
...
```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\smoke-server.ps1 -BaseUrl http://127.0.0.1:14444
```
...
```bash
curl http://127.0.0.1:14444/health
```
...
- 确认 server 监听的是 `0.0.0.0:14444`，不是只监听 `127.0.0.1`。
- 确认使用的是这台 Mac 的局域网 IP 或域名。
- 在 macOS 防火墙或路由器上允许 `14444` 入站。
...
```powershell
.\target\debug\airpaste-server.exe --bind 0.0.0.0:14444 --db .\airpaste.redb
```

启用 token：

```powershell
.\target\debug\airpaste-server.exe --bind 0.0.0.0:14444 --db .\airpaste.redb --auth-token "<secret>"
```

或使用环境变量：

```powershell
$env:AIRPASTE_BIND = "0.0.0.0:14444"
...
```powershell
Invoke-RestMethod http://127.0.0.1:14444/health
```
...
Start-Process `
  ...
  -ArgumentList @("--bind", "0.0.0.0:14444", "--db", ".\airpaste.redb", "--auth-token", "<secret>")
```
...
- 确认 server 监听 `0.0.0.0:14444`。
- 确认客户端使用的是 Windows 机器的局域网 IP 或域名。
- 在 Windows Defender 防火墙中允许 `airpaste-server.exe` 或 TCP `14444` 入站。
...
| 参数 | 环境变量 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `--server-url` | `AIRPASTE_SERVER` | `http://127.0.0.1:14444` | server 地址 |
...
| `--clipboard-mode` | `AIRPASTE_CLIPBOARD_MODE` | `system` | `system` 与系统剪贴板同步；`isolated` 用 `Alt+C`/`Alt+V` 收发（托盘 GUI 默认 `isolated`） |
...
- server 是否监听 `0.0.0.0:14444`。
- 防火墙是否允许 server 端口。
- 启用 token 时，agent 是否传了同一个 `--auth-token`。
- server 健康检查是否成功：

```bash
curl http://<server-host>:14444/health
```

或 Windows：

```powershell
Invoke-RestMethod http://<server-host>:14444/health
```
...
- **设备 / 设备 ID**。
- **隔离模式**复选框：勾上后远端文本进“收件箱”而不覆盖系统剪贴板，配合 `Alt+C` / `Alt+V`
  发送/接收(**macOS 上 Alt 即 Option 键**,也就是 `Option+C` / `Option+V`)。
- **开机自启**复选框：macOS 写 `~/Library/LaunchAgents` 的 LaunchAgent；Windows 写注册表 `HKCU\…\Run`。
- **本机作为服务器**复选框:在本机直接跑一个控制平面服务器(默认 `0.0.0.0:14444`,数据库放
  `…/AirPaste/server.redb`),供其它设备连接,不用单独开 `airpaste-server`。勾选后状态栏显示运行中/失败;
  会持久化,下次启动自动起(且在 agent 连接前就绪)。本机 agent 默认就连 `127.0.0.1:14444`,所以勾上即用。
- **设置 / 连接** 面板:填**服务器地址 / 设备名称 / 配对码 / 认证令牌 / 发送热键 / 粘贴热键**(热键
  留空即默认 `Alt+C` / `Alt+V`,格式见第 5 节 `--hotkey-copy`),点 **保存并连接**(配置持久化到
  `…/AirPaste/tray-config.json`,下次启动自动连接;配对码用过一次后自动清掉;保存时会重启应用以应用新配置)。
  面板里还有 **生成配对码** 按钮(已连接时可用):在**可信设备**上点一下出一个 6 位数字码(10 分钟有效),
  拿到新设备的「配对码」里填即可——**给新设备入网不用再碰命令行**。
- **发送文字到其它设备**:在输入框里输入或粘贴文字,点 **发送** 直接发布(端到端加密;显式发送不做
  敏感内容过滤)。成功显示 ✓ 并清空输入框;失败保留草稿可重试。
- **拖文件发送**:把文件/文件夹直接**拖进窗口**即可发送(悬停时窗口出现"松开以发送文件"提示),
  效果等同于"复制了文件"——对方按 `Alt+V` 或在收件箱点「下载」接收。大文件松手后会先显示
  「文件处理中…」(计算校验和),再显示「✓ 文件已发出」。
- **收件箱**:文本和文件混合显示,最新在上。文本条目点「复制」放入剪贴板;文件条目点「**下载**」——
  走与 `Alt+V` 相同的直连→中继路径,完成后文件引用自动放入剪贴板可直接粘贴,按钮随即变为「复制」
  (之后再点是从本地缓存重新放入剪贴板,不会重新传输);失败会显示原因并提供「重试」。
  `Alt+V` 拉取过的文件条目会自动标记为已下载。注意:文件的下载令牌默认 10 分钟有效,过期条目需对方重发。
  `Alt+V` 仍按**最近到达**优先(文本或文件);下载多/大文件时窗口显示进度条。
...
### 14.4 纯 GUI 上手流程(不碰命令行)

两台设备(A、B),A 兼作服务器:

1. **A:** 打开 AirPaste 托盘 → 设置面板填服务器地址(本机作服务器就保持默认 `http://127.0.0.1:14444`)→
   勾 **本机作为服务器** → 点 **保存并连接**。A 是第一台,自动信任 → `● 已连接`。
   (macOS 记得在 系统设置 → 隐私与安全性 → 辅助功能 里授权启动托盘的程序,否则热键无效。)
2. **A:** 在设置面板点 **生成配对码**,记下那 6 位数字码。
3. **B:** 打开托盘 → 设置面板填服务器地址(A 的局域网 `http://A的IP:14444`)+ 把配对码填进 **配对码** →
   **保存并连接** → `● 已连接`。(macOS 同样授权辅助功能。)
4. **A 发:** 选中文字,按 `Alt+C`(**macOS 上是 `Option+C`**)。**B 收:** 点进输入框,按 `Alt+V`(macOS `Option+V`)。
   也可在 B 的「收件箱」点「复制」。文件/文件夹同理:在 A 复制,B 按 `Alt+V` 下载(带进度条)。
   完全不用热键也行:A 在窗口里输入文字点「发送」,或把文件**拖进窗口**;B 在「收件箱」对文本点「复制」、
   对文件点「下载」。

全程点鼠标即可,不需要命令行。想让两台都开机自启就各自勾 **开机自启**。
