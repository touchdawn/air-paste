...
Air Paste 是一个基于 Rust 的跨设备共享剪贴板工具：在设备 A 复制文本或文件，在设备 B 按一个快捷键就能粘贴。支持 **Windows、macOS**，以及通过快捷指令接入的 **iPhone**（文本收发，无需装 App）。文本端到端加密；文件在局域网内点对点传输（无法直连时自动回退到加密的服务器中继），服务器永远不存储文件内容。
...
- **iPhone**：不用装 App，两个快捷指令（建议绑定「轻点背面」二击/三击）即可上传/下载文本，体验相当于手机版的 `Option+C` / `Option+V`，见下文「接入 iPhone」。
